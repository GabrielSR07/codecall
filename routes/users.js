...
router.post('/register', [
  body('name').not().isEmpty().withMessage("You must input a name"),
  body('email').isEmail().withMessage("Invalid or empty email"),
  body('password').not().isEmpty().isLength({ min: 5 }).withMessage("You must input your password"),
],
  ...
  try{
      ...
      bcrypt.hash(password, salt, async function(err, hash) {
        ...
        if(!err){
          ...
          errors = [];
          errors.push({msg: "An error ocurred", param: 'email'});
          return res.render('register', {
            title: 'Register',
            'name': name,
            'email': req.body.email,
